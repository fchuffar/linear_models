...
Remarques : 
  ...
  - La régression linéaire se caractérise par des variables explicatives **quantitatives** (*e.g.*, $poid$ \sim $taille$). L’ANOVA se caractérise par des variables explicatives  **qualitatives** (*e.g.*, $poids \sim sexe$).
...
Par ailleurs, en raison des conditions d’application du test, $F_{obs}$ suit une la loi de Fisher à $p-1$ et $n-p$ dégrés de liberté notée $\mathcal{F}_{p-1,n-p}$.
...
5. P-valeur associée
6. Conclusion du test
7. Commenter le pourcentage de variance expliqué ($R^2$)
