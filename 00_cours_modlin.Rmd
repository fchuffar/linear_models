...
R (1993) est un **logiciel** libre  sous licecnce [GPL](https://en.wikipedia.org/wiki/Free_software#Definition_and_the_Four_Essential_Freedoms_of_Free_Software).
...
- Réaliser un clone ou fork du package `fonctionr` que vous renommerez `monanalyse` et dont vous serez l’auteur (fichier DESCRIPTION)
...
- Insérer dans la vignette existante du package un paragraphe en `markdown` qui fait un appel à la fonction `plot_model` (fichir vignettes/make_a_coffee.Rmd)
- Vérifier (`devtools::check()`) votre package et construisez le. 
