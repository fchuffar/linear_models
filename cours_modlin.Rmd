---
title: "Modèles linéaires et GLM"
author: "Florent Chuffart & Magali Richard"
date: "`r Sys.Date()`"
output: 
  rmarkdown::html_document:
    toc: true
    toc_float: true
    toc_depth: 3
    number_sections: true
---


```{r, echo=FALSE, eval=TRUE}
knitr::opts_chunk$set(collapse=TRUE, comment = "#>", fig.width=9, fig.height=6, eval=TRUE, echo=FALSE, results="hide")
# README RStudio config, uncheck: # preferences > R Markdown > show output inline for... 
```

# Avant Propos



## Les plateformes

1. __GITHUB__ Le cours est dés maintenant disponible sur github, il évoluera au fil des sessions, le dépot sera régulièrement mis à jour  
  https://github.com/fchuffar/linear_models

2. __CODALAB__ Les TP seront organisés dès la première session autour de plusieurs data challenges sur la plateforme codalab, je vous invite à vous connecter ici maintenant pour gagner du temps
  https://competitions.codalab.org/competitions/28092?secret_key=6d097f54-b525-4fa7-849c-5f31e142f848

3. __GOOGLEFORM__ Certaines évaluation de contrôle continu seront réalisées via des questionnaires Google.

[Questionnaire Séance 1](https://forms.gle/tk5n2FZVba4iv28j7)

## Evaluations

 - Individuelle via un questionnaire à certaines séances.
 - En équipe lors d’un *data challenge* séance 4.

## Pré-requis
 
 - R https://cran.r-project.org 
 - RStudio https://www.rstudio.com
 - git and github https://r-pkgs.org/git.html & https://privefl.github.io/R-presentation/rstudio-git.html#1 

 ## Organisation prévisionnelle

  - Séance 1 : Régression linéaire simple
  - Séance 2 : Régression linéaire multiple
  - Séance 3 : ANOVA 
  - Séance 4 : [*Gene Expression Prediction Challenge 3.0 (expred3.0)*] (https://competitions.codalab.org/competitions/28931?secret_key=156ec53c-3ae2-4788-ac69-1eb11c467910) 
      [Compte rendu de TP prérempli](https://github.com/fchuffar/starting_kit_expred3.0)

  
---





```{r child = "01_cours_modlin.Rmd", eval=TRUE}
```

```{r child = "02_cours_modlin.Rmd", eval=TRUE}
```
```{r child = "02.1_cours_modlin.Rmd", eval=TRUE}
```

```{r child = "03_cours_modlin.Rmd", eval=TRUE}
```

```{r child = "04_cours_modlin.Rmd", eval=TRUE}
```


---

# Références et notes

François Rebaudo, (2021).
*R pour les scientifiques*, **Dunod**, France.

Daudin Jean-Jacques, (2015).
*Le modèle linéaire et ses extensions*, **ellipse**, France.
http://moulon.inra.fr/modelstat/supports/ModeleLineaireEt_Extensions-compressed.pdf

Cornillon, P.-A. et Matzner-Løber, E. (2011). *Régression avec R*, **Springer**, France.

Cornillon, P.-A. et Matzner-Løber, E. (2007). *Régression. Théorie et applications*, **Springer**, France.

McCullagh, P. et Nelder, J.A. (1989). *Generalized Linear Models. Second Edition. Monographs on Statistics and Applied Probability* 37. **Chapman and Hall**.

Madsen H. and Thyregord, P (2011). *Introduction to General and Generalized Linear Models*, **CRC Press**, Taylor and Francis Group, Boca Raton, Florida, USA.

Weisberg, S (2005). *Applied linear regression*. Vol. 528. **John Wiley & Sons**.


Ce cours s’inspire des échanges avec les personnes et liens suivants : 

- Frédéric Bertrand & Myriam Maumy-Bertrand
- Franck Picard
- Olivier François
- http://math.agrocampus-ouest.fr (janvier 2020)
- https://delladata.fr/introduction-puissance-statistique (décembre 2020)
- https://andrewlauactuary.com/2019/06/04/shiny-app-showing-statistical-power (juin 2019)

---

```{r, results="verbatim"}
sessionInfo()
```



