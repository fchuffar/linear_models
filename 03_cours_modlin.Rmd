...
avec : 
  ...
  -  la *matrice d’incidence* (ou tableau disjonctif) de taille $n$ x $p$, ne comportant que des 0 et des 1 et permettant d’associer chaque observation à son niveau de facteur $X$.   
...
Dans notre exemple $\beta_i$ prend ses valeurs dans $\{ \beta_A, \beta_B, \beta_C, \beta_D, \beta_E, \beta_F \}$, les moyennes des observations pour les individus des groupes A, B, C, D, E et F. Les groupes sont équilibrés donc $\beta_A + \beta_B + \beta_C + \beta_D + \beta_E + \beta_F = 0$ 
...
- $k$ identifie la *k*-éme observation pour un niveau de facteur donné (dans notre exemple, pour chaque type d’insecticide, $k \in K = \{1,...,12\}$).
...
Le Test de l'ANOVA à 1 facteur (*one way ANOVA*), s'applique sous certaines condition : 
...
m = lm(coag~diet, d[d$diet %in% c("A", "B"),])
summary(m)
anova(lm(coag~diet, d[d$diet %in% c("A", "B"),]))
