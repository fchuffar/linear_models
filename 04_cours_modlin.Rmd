...
**Notations**
  ...
  - $y_{ij}$ le rendement de la variété avec le fongicide $j$.
...
**Fisher** : test global de l’effet d’un facteur
...
Si les conditions d’aplications sont satisfaites et sous $H_0$ alors $F_{obs}$ est une réalisation d’une variable aléatoire $F$ qui suit une loi de Fisher à $I-1$ degrés de liberté au numérateur et $n-I-J+1$ degrés de liberté au dénominateur. Cette loi est notée  $\mathcal{F}_{I-1,n-I-J+1}$.
...
plot(x,df(x, ddl_f2, ddl_res), type="l", main=paste0("F_", ddl_f2, "_", ddl_res, ", Fobs_F1=", f_obs_f2, ", P(>F)=", signif(1-pf(f_obs_f2,ddl_f2, ddl_res),3)), ylab=paste0("df(x, ", ddl_f2, ", ", ddl_res, ")"))
...
**Student** : test de la significativité d’un coefficient 
...
Si les conditions d’aplications sont satisfaites et sous $H_0$ alors la valeur absolue de $t_{obs}$ est une réalisation d’une variable aléatoire $t$ qui suit une loi de Student à $n-I-J+1$ degrés de liberté. Cette loi est notée  $\mathcal{t}_{n-I-J+1}$.
...
Etudiez les jeux de données suivants : 
